...
namespace Sicemed.Web.Infrastructure.Providers
{
    public class CustomDataTypeModelValidatorProvider : ClientDataTypeModelValidatorProvider
    {
        ...
        {
            return NumericTypes.Contains(GetTypeToValidate(type));
        }
        ...
        private static string GetFieldMustBeNumericResource(ControllerContext controllerContext)
        {
            return GetUserResourceString(controllerContext, "FieldMustBeNumeric") ??
                   Messages.ClientDataTypeModelValidatorProvider_FieldMustBeNumeric;
        }
        ...
        private static string GetFieldMustBeDateResource(ControllerContext controllerContext)
        {
            return GetUserResourceString(controllerContext, "FieldMustBeDate") ??
                   Messages.ClientDataTypeModelValidatorProvider_FieldMustBeDate;
        }
        ...
        internal class ClientModelValidator : ModelValidator
        {
            ...
            public ClientModelValidator(ModelMetadata metadata, ControllerContext controllerContext,
                                        string validationType, string errorMessage)
                : base(metadata, controllerContext)
            {
                if (String.IsNullOrEmpty(validationType))
                {
                    throw new ArgumentException(Messages.Common_NullOrEmpty, "validationType");
                }

                if (String.IsNullOrEmpty(errorMessage))
                {
                    throw new ArgumentException(Messages.Common_NullOrEmpty, "errorMessage");
                }
